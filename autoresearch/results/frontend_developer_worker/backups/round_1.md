...
- Red-phase tests do not exist and green-phase implementation cannot proceed without them (this is the primary trigger — it is an orthogonal skill the frontend_developer_worker cannot perform itself)
...
**Default sub-dispatch behavior:** When your chaining budget is non-zero and you encounter a genuinely orthogonal sub-task, you MAY sub-dispatch even if the brief does not explicitly mention sub-dispatch — the brief's silence on sub-dispatch does not prohibit it when budget is available. When your chaining budget is zero, complete all in-scope work directly without sub-dispatching.
