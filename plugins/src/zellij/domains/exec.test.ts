...
async function unitTests() {
  ...
  execFileSync("mkfifo", [`${dir}/cmd.pipe`])
...
// ---------------------------------------------------------------------------
// Stale Bridge Recovery Tests (needs Zellij for start)
// ---------------------------------------------------------------------------

async function staleBridgeTests() {
  const sessionName = process.env.ZELLIJ_SESSION || ""

  if (!sessionName) {
    console.log("\n=== STALE BRIDGE TESTS: SKIPPED (set ZELLIJ_SESSION=<name>) ===\n")
    return
  }
  ...
  try {
    ...
    if (!sessions.includes(sessionName)) {
      console.log(`\n=== STALE BRIDGE TESTS: SKIPPED (session "${sessionName}" not found) ===\n`)
      return
    }
  } catch {
    console.log("\n=== STALE BRIDGE TESTS: SKIPPED (zellij not available) ===\n")
    return
  }

  console.log(`\n=== STALE BRIDGE TESTS: crash recovery (session: ${sessionName}) ===\n`)

  const { handleExec } = await import("./exec.js")

  // Ensure no bridge running
  await handleExec("stop", { session: "stale-test" }).catch(() => {})

  // Simulate crash: create stale bridge dir with FIFO + fake pane.id, no PID file
  const crashDir = "/tmp/zellij-bridge-stale-test"
  try { rmSync(crashDir, { recursive: true, force: true }) } catch {}
  mkdirSync(`${crashDir}/results`, { recursive: true })
  execFileSync("mkfifo", [`${crashDir}/cmd.pipe`])
  writeFileSync(`${crashDir}/pane.id`, "terminal_99999") // nonexistent pane

  // Verify the stale FIFO exists
  assert(existsSync(`${crashDir}/cmd.pipe`), "stale FIFO exists before recovery")

  // Now start() should clean this up and succeed
  {
    const r = await handleExec("start", { session: "stale-test", direction: "down" })
    assert(r.includes("Bridge started"), `crash recovery start: ${r.slice(0, 100)}`)
  }

  // Verify bridge is functional after recovery
  {
    const r = await handleExec("run", { session: "stale-test", command: "echo recovered" })
    const parsed = JSON.parse(r)
    assertEq(parsed.stdout, "recovered", "crash recovery: bridge functional after stale cleanup")
  }
...
async function integrationTests() {
  ...
  // Start bridge in a new pane
  {
    ...
    assert(r.includes("Bridge started") || r.includes("Bridge already running"), `start: ${r.slice(0, 100)}`)
    assert(r.includes("pane "), `start includes pane ID: ${r.slice(0, 100)}`)
  }

  // Status check
  {
    const r = await handleExec("status", { session: sessionName })
    assert(r.includes("alive") || r.includes("Recent output"), `status: ${r.slice(0, 100)}`)
  }
  ...
  // Restart after stop — no mkfifo EEXIST error
  {
    const r = await handleExec("start", { session: sessionName, direction: "down" })
    assert(r.includes("Bridge started"), `restart after stop: ${r.slice(0, 100)}`)
    await handleExec("stop", { session: sessionName }).catch(() => {})
  }
}
